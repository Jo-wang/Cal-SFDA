...
def Res_Deeplab_encoder(num_classes=21):
    model = ResNet_encoder(Bottleneck, [3, 4, 23, 3], num_classes)
    return model
...
class ResNet_decoder(nn.Module):
    ...
    def forward(self, x, H, W, reverse=False):
        if reverse:
...
def Res_Deeplab_decoder(num_classes):
    model = ResNet_decoder(Bottleneck, [3, 4, 23, 3], num_classes)
    return model
